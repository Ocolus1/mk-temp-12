    ...
    <div className="font-medium">
      <Listbox value={selected} onChange={setSelected}>
        <div className="relative">
          <Listbox.Button className="relative w-full cursor-pointer rounded-full bg-white border border-slate-500/20 p-2.5 px-3 text-left focus:outline-none focus:ring focus:ring-indigo-500/50 text-base">
            <span className="block truncate">
              {selected === defaultOption ? (
                <span className="text-slate-500/40 text-sm">
                  {selected.name}
                </span>
              ) : (
                selected.name
